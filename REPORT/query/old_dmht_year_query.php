...
FROM
hi.grdmht
LEFT JOIN (SELECT ovstdx.icd10, ovstdx.vn, ovst.hn, ovst.vstdttm FROM hi.ovst LEFT JOIN hi.ovstdx ON ovst.vn = ovstdx.vn WHERE date(ovst.vstdttm) BETWEEN '$sy10' AND '$ey09' ) AS dx
ON grdmht.icd10 = dx.icd10
...
,sum(if(MONTH(dx.vstdttm)=9,1,0)) as visit_l
,COUNT(DISTINCT IF(date(dx.vstdttm) BETWEEN '$sy10' AND '$ey09' , dx.hn, null)) as human_total
,sum(DISTINCT IF(date(dx.vstdttm) BETWEEN '$sy10' AND '$ey09' , dx.vn, null)) as visit_total
FROM
hi.grdmht
LEFT JOIN (SELECT ovstdx.icd10, ovstdx.vn, ovst.hn, ovst.vstdttm FROM hi.ovst LEFT JOIN hi.ovstdx ON ovst.vn = ovstdx.vn WHERE date(ovst.vstdttm) BETWEEN '$sy10' AND '$ey09' ) AS dx
ON grdmht.icd10 = dx.icd10
