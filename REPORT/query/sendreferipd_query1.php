...
select  z.accdate,z.can,z.hn,z.rfrlct,z.vn,hospcode.off_name1  as cname,hospcode.off_id  
 
from(SELECT hi.ovstdx.icd10, 
	hi.ovst.hn, 
	count(hi.ovst.vn) AS can, 
	 
	hi.ovst.vn, 
	hi.ovst.vstdttm AS accdate, 
	 
	hi.orfri.rfrlct
FROM hi.ovstdx INNER JOIN hi.ovst ON hi.ovst.vn = hi.ovstdx.vn
	 INNER JOIN hi.orfri ON hi.orfri.vn = hi.ovst.vn
WHERE ovstdx.cnt = 1 and   ovst.vstdttm between '$start' and '$end'
GROUP BY vn
ORDER BY hi.ovst.hn DESC ) as z INNER JOIN hospcode ON  hospcode.off_id =z.rfrlct)as d  group by  d.rfrlct  ORDER BY total DESC
	    "; 
