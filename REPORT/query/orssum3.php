...
SELECT 
	...
	ors.opdate  as yeaript, 
 
	idpm.nameidpm,
     ipt.ward 
FROM ipt INNER JOIN ors ON ipt.vn = ors.vn AND ipt.an = ors.an AND ipt.hn = ors.hn
	 INNER JOIN idpm ON idpm.idpm = ipt.ward
where year(ors.opdate) between '$y4' and  '$Y'    and  ipt.an >  '0' GROUP BY can ORDER BY can desc) as a  GROUP BY  a.ward  ORDER BY ddd desc
";
