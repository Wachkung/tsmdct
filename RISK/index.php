												...
												<table id="ticker-table" class="table m-table table-bordered table-hover table-heading">
													...
													<tbody>
															<?php
															    //จำนวนแยกตามโปรแกรม															
                                                                $i=1;
																$sql1 = "SELECT risk2_datarisk.fromevent,risk2_group.namegroup,COUNT(risk2_datarisk.id) FROM risk2_datarisk JOIN risk2_group on risk2_group.codegroup=risk2_datarisk.fromevent WHERE daterigter BETWEEN '".$strdate."'and '".$enddate."' GROUP BY risk2_group.namegroup ORDER BY risk2_datarisk.fromevent ASC";
																$objQuery = mysql_query($sql1); while($objResult = mysql_fetch_array($objQuery)) {
															?>
														<tr>
															<td class="m-ticker">(P<?php echo $i;?>) <?=$objResult['1'];?></td>
															<td class="m-price" align="center"><a href="report_group.php?fromevent=<?=$objResult['0'];?>"><?=$objResult['2'];?></a></td>
														</tr>
                                                        <?php $i++; }?>
													</tbody>
												</table>
