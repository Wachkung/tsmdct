    ...
    <?php  include "navigator.php";?>
            ...
            <aside class="right-side">
                ...
                <section class="content-header">
                    <h1>
                        รายการครุภัณฑ์ IT อื่นๆ
                    </h1>
                    <ol class="breadcrumb">
                        <li><a href="index.php"><i class="fa fa-dashboard"></i> หน้าแรก</a></li>
                        <li><a href="device.php">ครุภัณฑ์</a></li>
                        <li class="active">ครุภัณฑ์ IT อื่นๆ</li>
                    </ol>
                </section>
                        ...
                        <div class="col-xs-12">
                            <div class="box box-solid box-primary">
                                <div class="box-header">
                                    <h3 class="box-title">ข้อมูลรายการครุภัณฑ์ IT อื่นๆ</h3>
                                </div><!-- /.box-header -->
                                
                                <div class="box-body">
                                    <div class="form-group">
                                    <button type="button" class="btn btn-danger" onclick="window.location.href='main1.php'">
                                    <i class="fa fa-reply"></i> กลับหน้าครุภัณฑ์</button>
                                    <button type="button" class="btn btn-success" 
                                    onclick="window.location.href='device_add.php'">
                                    <i class="fa fa-plus"></i> 1.เพิ่มรายการครุภัณฑ์ IT อื่นๆ</button>	
                                    </div><!-- /.form group -->
                                </div>
                                ...
                                <div class="box-footer table-responsive">
                                    <table id="example1" class="table table-bordered table-hover">
                                        <thead>
                                            <tr>
                                            	<th width="5%">No.</th>
                                                <th width="10%">เลขครุภัณฑ์</th>
                                                <th width="12%">ประเภทอุปกรณ์</th>
                                                <th width="8%">ยี้ห้อ</th>
                                                <th width="20%">รายละเอียด</th>
                                                <th width="15%">สถานที่ติดตั้ง</th>
                                                <th width="8%">สถานะ</th>
                                                <th width="5%">ให้บริการ</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                        <?php    
											$no=1;
											$sql=" SELECT * FROM it_device  order by id DESC "; 
										...
										?>
                                            <tr>
                                                ...
                                                <td><?=$data['serial']?></td>
												<?php
													$strSQLdtype = "SELECT * FROM durable WHERE durable_code = '$dtype'";
													$objQuerydtype = mysql_query($strSQLdtype);
													$objResultdtype = mysql_fetch_array($objQuerydtype);
												?>

                                                <td><?=$objResultdtype['durable_name']?></td>
                                                <td><?=$data['brand']?></td>
                                                <td><?=$data['spec']?></td>
												<?php
													$strSQLdepart = "SELECT * FROM risk2_departreport WHERE CODE = '$depart'";
													$objQuerydepart = mysql_query($strSQLdepart);
													$objResultdepart = mysql_fetch_array($objQuerydepart);
												?>
                                                
                                                <td><?=$objResultdepart['DeptName']?></td>
                                                <td><?=$data['status']?></td>
                                                <td><a href="device_detail.php?device_code=<?=$data['device_code']?>" title="<?=$data['device_code']?>"><span class="btn bg-green">เลือก</span></a></td>
                                            </tr>
                                        <?php  $no++; } ?>
                                        </tbody>
