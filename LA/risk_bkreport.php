<?php
	...
	$strSQLuser = "SELECT risk2_departreport.DeptName FROM person INNER JOIN risk2_departreport on rm_depart.`CODE` = person.depart WHERE idcard = '$IDCARD1' ";
	$objQueryuser = mysql_query($strSQLuser);
	$objResultuser = mysql_fetch_array($objQueryuser);
	$depart=$objResultuser["DeptName"];
                        ...
                        <div class="col-xs-12">
                            <div class="box">
                                <div class="box-header">
                                    <h3 class="box-title">รายงานความเสี่ยงส่งมาหน่วยงาน</h3>
                                </div><!-- /.box-header -->
                                    ...
                                    <table id="example1" class="table table-bordered table-hover">
                                        <thead>
                                            <tr>
                                                ...
                                                <th width="15%">หน่วยงานที่รับผิดชอบ</th>
                                                <th width="10%">ประสบปัญหา</th>
                                                <th width="10%">บันทึกข้อมูล</th>
                                        ...
                                        <?php  
											$total=0; $no=1;
											$sql=" SELECT * FROM risk2_datarisk where departrespon = '$depart' order by daterigter DESC "; 
                        					$result = mysql_query($sql); while($data = mysql_fetch_array($result)) {
										?>
                                            <tr>
                                            	<td><?=$no?></td>
                                                <td><?=$data['dataevent']?></td>
                                                <td><?=$data['notepatient']?></td>
                                                <td><?=$data['departrespon']?></td>
                                                <td><?=$data['daterigter']?></td>
                                                <td><?=$data['datereport']?></td>
                                               <td><a href="risk_bkdetail.php?id=<?=$data['id']?>" title="<?=$data['dataevent']?>"><span class="label label-primary"><i class="fa fa-edit"></i></span></a></td>
                                            </tr>
                                        <?php $no++; } ?>
                                        </tbody>
                                        <tfoot>
